...
## Should really be done just as different option in 1 script for import/compile/run/getstatus


## For the import.. we could write something to find all the isx files to import..
##  or maybe allow passing in multiple ( how many can you pass in?)
...
def main():
    ...
    logMessage.info("Starting the isx import script.")
    ...
    p = {
        "api": "importAssets",      
        "engineName" : args.hostName  ,
        "domain": args.serviceTierHostName, 
        "projectName" : args.projectName , 
        "replace" : 'yes',
        "includeDesign" : 'yes' ,
        "remoteFilePath": '/var/git/test1_repo/code_sheff/DataStageJobsTest1/dstage1/SteTest/RunCommand4/RunCommand4.isx' ,
        "apiVersion" : "3.0"
        } 
        ...
        for job in jobs:
            ...
            if response.status_code != 200:
                ...
                if response.status_code == 404:
                    logMessage.error('status code 404 probably means something. for url : ' + url + '. Check you have entered username and password correctly.')
                    break
        

            if response.json()['succeeded'] == True:
                logMessage.info(job + ' imported ok.')
            else:
                logMessage.warning(job + ' import FAILED.' + str(response.json()['failureMessage']) + ' ( If "Null RID" then the combination of hostName,project and jobname does not exist.) ')
